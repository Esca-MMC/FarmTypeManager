...
MapName | Any map name, e.g. "Farm" | The name of the in-game location where objects will spawn. | Go to an in-game location and use the `whereami` command in SMAPI's console to view its map name (see [Commands](#commands)). Multiple location names can be separated by spaces, e.g. `"Farm, BusStop"`. Using a building name such as "Deluxe Barn" will spawn objects in every copy of that building. Start a name with "prefix:", "suffix:", or "contains:" to match text in multiple location names; for example, "prefix:Farm" targets Farm, Farmhouse, FarmCave, etc.
