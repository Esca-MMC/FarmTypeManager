...
CanBePickedUp | No | **true**, false | When set to false, players will not be allowed to pick up this item. "True" has no effect. | Furniture and big craftables should still be useable, but cannot be picked up. All items with this setting will be removed overnight by default; respawn them daily or use the "DaysUntilSpawnsExpire" setting. This setting has no effect on containers or monster loot.
