...
namespace FarmTypeManager.TileQueries
{
    ...
    public class NotIndexTileQuery : ITileQuery
    {
        ...
        public NotIndexTileQuery(GameLocation location, string[] queryArgs)
        {
            Location = location;
        ...
        /// <summary>A set of invalid index values. Null if all values are invalid.</summary>
        ...
        public bool CheckTile(Vector2 tile) => !(Location.getTileIndexAt((int)tile.X, (int)tile.Y, Layer) is int tileIndex && tileIndex >= 0 && (IndexValues == null || IndexValues.Contains(tileIndex))); //tile does not have an index on the layer (or, if specified, it doesn't match)
