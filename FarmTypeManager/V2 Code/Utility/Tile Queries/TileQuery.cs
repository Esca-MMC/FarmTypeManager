...
{
    /// <summary>A top-level tile query. Outputs tiles that are considered valid by all queries in a tile condition.</summary>
    public class TileQuery
    {
        ...
        /// <param name="tileCondition">A set of tile queries and their arguments, separated by commas.</param>
        public TileQuery(GameLocation location, string tileCondition)
        {
        ...
        private string TileCondition { get; set; }

        /// <summary>A list of tile queries created by parsing <see cref="TileCondition"/>.</summary>
        ...
        public IEnumerable<Vector2> GetTiles(bool randomizeOrder = true)
        {
            ...
            List<Vector2> tiles;
            if (ChooseStartingTilesSource() is ITileQuery startingQuery) //if a query should generate a starting list
            {
        ...
        private ITileQuery ChooseStartingTilesSource()
        {
            ...
            FTMUtility.Monitor.Log($"TEST: starting list query type: {highestQuery?.GetType()}", StardewModdingAPI.LogLevel.Alert);
            return highestQuery;
        }
        ...
        private void Initialize()
        {
            ...
            string[] conditions = ArgUtility.SplitQuoteAware(TileCondition, ',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries, true);

            foreach (string condition in conditions)
            {
                string[] args = ArgUtility.SplitBySpaceQuoteAware(condition); //split condition into arguments around spaces (and remove empty entries)
