...
namespace FarmTypeManager.CustomActions
{
    ...
    public static class ITimesToPerformSettingsExtensions
    {
        ...
        /// <param name="random">The random number generator to use when checking conditions. If not provided, <see cref="Properties.Random"/> will be used. Note that <see cref="Properties.Random"/> will always be used in other logic; this argument only affects conditions.</param>
        ...
        {
            int startingNum = Properties.Random.Next(settings.MinTimes, settings.MaxTimes + 1); //get a random number between min and max (inclusive)

            if (settings.TimesModifiers?.Count > 0)
            {
                float modifiedNum = Utility.ApplyQuantityModifiers(startingNum, settings.TimesModifiers, settings.TimesModifierMode, location, player, targetItem, inputItem, random ?? Properties.Random); //apply modifiers with any provided GSQ context
